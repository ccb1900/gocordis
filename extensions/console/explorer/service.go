...
// New creates the Service. nameOfType maps a component type to a human
// label; nil falls back to the raw type, and applications inject their own
// vocabulary.
func New(nameOfType func(string) string) *Service {
	if nameOfType == nil {
		nameOfType = func(t string) string { return t }
	}
	return &Service{protected: map[string]string{}, nameOfType: nameOfType}
}
...
func (s *Service) Plugins() []Plugin {
	...
	for _, cc := range s.desired {
		p := Plugin{
			ID:           cc.ID,
			Name:         s.nameOfType(cc.Type),
			Type:         cc.Type,
			State:        "Gone",
			Components:   []string{},
			Capabilities: []string{},
			Controllable: s.protected[cc.ID] == "",
			Config:       scalarConfig(cc.Config),
		}
...
func scalarConfig(cfg map[string]any) map[string]string {
	...
	for k, v := range cfg {
		...
		case float64:
			out[k] = fmt.Sprint(t)
		case []any:
			out[k] = fmt.Sprintf("(%d entries)", len(t))
		case map[string]any:
			out[k] = fmt.Sprintf("(%d keys)", len(t))
		}
	}
	return out
}
